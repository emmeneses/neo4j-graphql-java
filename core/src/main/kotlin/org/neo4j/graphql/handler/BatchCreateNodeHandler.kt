...
class BatchCreateNodeHandler private constructor(schemaConfig: SchemaConfig) : BaseDataFetcherForContainerBatch(schemaConfig) {
    ...
    class Factory(schemaConfig: SchemaConfig,
        ...
        override fun createDataFetcher(operationType: OperationType, fieldDefinition: FieldDefinition): DataFetcher<Cypher>? {
            ...
            val type = (fieldDefinition.type.inner().resolve() as ImplementingTypeDefinition).fieldDefinitions.singleOrNull()?.type?.inner()?.resolve() as? ImplementingTypeDefinition<*>
                    ?: return null
    ...
    override fun generateCypher(variable: String, field: Field, env: DataFetchingEnvironment): Statement {
        ...
        val input = field.arguments.find { it.name == INPUT }
        val inputs = (input?.value as? ArrayValue)?.values
                ?: throw IllegalArgumentException("expected an input of type ${env.fieldDefinition.getArgument(INPUT).type} but got ${input}")

        var queries: StatementBuilder.OngoingReadingWithoutWhere? = null
        val nodes = mutableListOf<Node>()
        inputs.forEachIndexed { index, value ->
            val properties = properties(variable, value as ObjectValue)
        ...
        val targetField = field.selectionSet.selections.find { (it as? Field)?.name == targetArrayField } as Field
        val mapProjection = projectFields(node, targetField, type, env)
        return result
            .returning(
                    mapOf(
                            targetArrayField,
                            Functions.collect(node.project(mapProjection))
                    )
